...
#旨在通过多尺度卷积和跨特征注意力机制实现输入特征的高效融合与增强
def to_3d(x):
...
#LayerNorm 对输入特征进行归一化，有两种实现方式：BiasFree_LayerNorm 和 WithBias_LayerNorm
...
#LayerNorm 通过归一化使输入特征在数值上更稳定，改善网络收敛
...
class MDAF(nn.Module):  # 多尺度卷积和跨特征注意力机制
    def __init__(self, dim, num_heads, LayerNorm_type):
        ...
        # 卷积核尺寸修改为 3, 5, 7，并确保 padding 保持特征图大小不变
        self.conv1_1_1 = nn.Conv2d(dim, dim, (1, 3), padding=(0, 1), groups=dim)
        self.conv1_1_2 = nn.Conv2d(dim, dim, (1, 5), padding=(0, 2), groups=dim)
        self.conv1_1_3 = nn.Conv2d(dim, dim, (1, 7), padding=(0, 3), groups=dim)
        self.conv1_2_1 = nn.Conv2d(dim, dim, (3, 1), padding=(1, 0), groups=dim)
        self.conv1_2_2 = nn.Conv2d(dim, dim, (5, 1), padding=(2, 0), groups=dim)
        self.conv1_2_3 = nn.Conv2d(dim, dim, (7, 1), padding=(3, 0), groups=dim)

        self.conv2_1_1 = nn.Conv2d(dim, dim, (1, 3), padding=(0, 1), groups=dim)
        self.conv2_1_2 = nn.Conv2d(dim, dim, (1, 5), padding=(0, 2), groups=dim)
        self.conv2_1_3 = nn.Conv2d(dim, dim, (1, 7), padding=(0, 3), groups=dim)
        self.conv2_2_1 = nn.Conv2d(dim, dim, (3, 1), padding=(1, 0), groups=dim)
        self.conv2_2_2 = nn.Conv2d(dim, dim, (5, 1), padding=(2, 0), groups=dim)
        self.conv2_2_3 = nn.Conv2d(dim, dim, (7, 1), padding=(3, 0), groups=dim)
